...
export default function OrganizationsAdmin() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold">Các Hội Đoàn & Ca Đoàn</h1>

      <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border shadow-sm">
        <h2 className="text-xl font-bold mb-4">{editingId ? 'Sửa Hội Đoàn' : 'Thêm Hội Đoàn Mới'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">Tên Hội Đoàn</label>
              <input required type="text" className="w-full border p-2 rounded" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="VD: Ca đoàn Thánh Tâm" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Bổn mạng</label>
              <input type="text" className="w-full border p-2 rounded" value={formData.patron} onChange={e => setFormData({...formData, patron: e.target.value})} placeholder="VD: Thánh Cecilia" />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Liên hệ (Đoàn trưởng)</label>
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Số lượng thành viên</label>
          ...
          <div className="flex gap-2 pt-2">
            <button type="submit" className="bg-teal-600 text-white px-4 py-2 rounded font-medium hover:bg-teal-700">Lưu Hội Đoàn</button>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border shadow-sm overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-zinc-50 dark:bg-zinc-800">
            <tr>
              <th className="p-4 font-medium">Tên Hội Đoàn</th>
          ...
          <tbody className="divide-y">
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={5} className="p-8 text-center text-gray-500">Chưa có hội đoàn nào.</td>
              </tr>
            )}
          </tbody>
        </table>
